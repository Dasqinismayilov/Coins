...
app.put('/all/:id', (req, res) => {
  const id = +req.params.id;
  const coin = req.body; // sikkə məlumatları sorğuya ötürülür

  connection.query(
    `UPDATE coins SET ? WHERE id = ?`,
    [coin, id], // sorğuda dəyərlər siyahısının ötürülməsi
    (err, result) => {
      if (!err) {
...
app.post('/all', (req, res) => {
  ...
  connection.query(
    ...
    coin, //sorğuda sikkə məlumatları olan bir obyektin ötürülməsi 
    (err, result) => {
      if (!err) {
